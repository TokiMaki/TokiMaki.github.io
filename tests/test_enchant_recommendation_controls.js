...
test('storage null은 기본 HTML과 기본-disabled 서약 상태를 보존한다', () => {
  ...
  assert.equal(
    ...
    '861c0f382ec040b4bd9d8b4e5f6c86f0bb268ae2a6bdfa1a054574d20bb4290e',
  );
...
test('legacy 유일 제작 체크 상태는 장비 유물로 이동한다', () => {
  const storage = new FakeStorage({
    [INCLUDE_KEY]: JSON.stringify(['마법부여:가성비', '유일:제작']),
    [KNOWN_KEY]: JSON.stringify([
      ...DEFAULT_INCLUDE_KEYS.filter((key) => key !== '장비:유물'),
      '유일:제작',
    ]),
  });
  const { controls, includeControls } = createHarness({ storage });
  ...
  assert.equal(getInput(includeControls, '장비:유물').checked, true);
  assert.equal(getInput(includeControls, '유일:제작'), undefined);
  assertStoredJson(storage, INCLUDE_KEY, ['마법부여:가성비', '장비:유물']);
  assertStoredJson(storage, KNOWN_KEY, DEFAULT_INCLUDE_KEYS);
});

test('legacy 유일 제작을 해제했던 사용자는 장비 유물도 해제 상태를 유지한다', () => {
  const storage = new FakeStorage({
  ...
  assert.equal(getInput(includeControls, '장비:유물').checked, false);
...
test('legacy fixture HTML 해시를 보존한다', () => {
  ...
  assert.equal(
    sha256(includeControls.innerHTML),
    '9076538cc4f89a8ef16d7f1321d69cc6ad3e5bc248c79ce7791945fb6006596d',
  );
});
