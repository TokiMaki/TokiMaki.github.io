...
test('storage null은 기본 HTML과 기본-disabled 서약 상태를 보존한다', () => {
  ...
  assert.equal(includeControls.innerHTML.length, 7540);
  assert.equal(
    sha256(includeControls.innerHTML),
    'ca0072971be6a3cfa13c4e749c61ce7a799ad3e368dd1dd274f8696eade609aa',
  );
  assert.equal(includeControls.inputs.length, 21);
  assert.equal(getInput(includeControls, '서약:초월/정가').checked, false);
  assert.equal(checkedValues(includeControls).length, 20);
...
test('legacy fixture HTML 해시를 보존한다', () => {
  ...
  assert.equal(
    sha256(includeControls.innerHTML),
    'c190a9393c997a0570471e9059d29adee8e4a615d0024232687a26a87f167202',
  );
});
