...
assert.match(source, /const selectedRoleInitializedRef = useRef\(false\)/);
assert.match(source, /if \(!selectedRoleInitializedRef\.current\)[\s\S]*?selectedRoleInitializedRef\.current = true/);
assert.match(source, /const selectRole = \(nextRole\) => \{[\s\S]*?selectedRoleInitializedRef\.current = true;[\s\S]*?setRole\(nextRole\)/);
