...
- 최근 캐시 구조: 전체 서버 후보 검색은 기존 `character-response-cache.sqlite`의 `character_search_candidate_cache` 테이블을 사용한다. 서버별로 `normalized_character_name + server_id` hit이면 검색 API를 건너뛰고, miss 서버만 API 검색 후 개별 후보 요약 row를 upsert하며 negative cache는 저장하지 않는다.
