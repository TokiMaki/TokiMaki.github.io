...
- 최근 프론트 구조: `enchantEfficiencyScale.js`는 효율 색상 계산, `enchantSearchPanels.js`는 분석·후보 패널 표시, `enchantCreatureRows.js`는 크리쳐·아티팩트 추천 row 정규화, `enchantEquipmentLoadoutBoard.js`·`enchantOathLoadoutBoard.js`·`enchantAvatarLoadoutBoard.js`·`enchantBuffLoadoutBoard.js`는 읽기 전용 장비판 HTML 생성, `enchantAvatarEmblemViewHelpers.js`는 avatar·buff 공용 엠블렘 표현 helper, `enchantPortraitDetailPanel.js`는 detail panel binding, `enchantLoadoutNavigation.js`는 탭 markup과 delegated click 전환, `enchantDevelopmentTiming.js`는 개발 timing 상태·step·flush, `enchantSimulatorDisplay.js`는 딜러·버퍼 시뮬레이터 점수 meta와 actions 표시를 담당한다. 기존 표시·row·reset·탭 fallback, timing ownership·stale·error/finally, simulator 계산·mutation·portrait 분기 계약은 유지한다.
- 다음 작업: 추천 include/filter/efficiency legend 렌더링의 DOM·localStorage·상태 의존성을 확인해 다음 분리 단위를 정한다.
