...
- 최근 프론트 구조: `enchantEfficiencyScale.js`는 효율 색상 계산, `enchantSearchPanels.js`는 분석·후보 패널 표시, `enchantCreatureRows.js`는 크리쳐·아티팩트 추천 row 정규화를 담당한다. 기존 표시·row 계약은 유지한다.
- 다음 작업: `enchantView.js`에서 서약 등 읽기 전용 장비판 하나를 다음 분리 대상으로 검토한다.
