...
- 최근 프론트 구조: `enchantEfficiencyScale.js`는 효율 색상 계산, `enchantSearchPanels.js`는 분석·후보 패널 표시, `enchantCreatureRows.js`는 크리쳐·아티팩트 추천 row 정규화, `enchantOathLoadoutBoard.js`·`enchantAvatarLoadoutBoard.js`·`enchantBuffLoadoutBoard.js`는 읽기 전용 장비판 HTML 생성, `enchantAvatarEmblemViewHelpers.js`는 avatar·buff 공용 엠블렘 표현 helper를 담당한다. 기존 표시·row·binder 계약은 유지한다.
- 다음 작업: 기존 코드 리뷰 기준으로 `enchantView.js`의 다음 분리 후보를 고르고 호출부·상태 변경·DOM 부작용 경계를 확인한다.
