...
- 최근 프론트 구조: `enchantEfficiencyScale.js`는 효율 색상 계산, `enchantSearchPanels.js`는 분석·후보 패널 표시, `enchantCreatureRows.js`는 크리쳐·아티팩트 추천 row 정규화, `enchantOathLoadoutBoard.js`는 읽기 전용 서약 장비판 HTML 생성을 담당한다. 기존 표시·row·binder 계약은 유지한다.
- 다음 작업: `enchantView.js`에서 아바타 등 다음 읽기 전용 장비판의 상태·렌더·binding 경계를 검토한다.
