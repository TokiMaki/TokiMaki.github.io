...
- 최근 데이터 작업: `Docs/switching_avatar_db.json`은 클래스 단위 스위칭 상하의 레어 아바타 후보명 DB다. 나무위키 1~14차 레어 아바타 키워드와 던파쇼룸 레어 등급 데이터를 대조해 후보명을 채웠고, `itemId`는 추후 보강을 위해 빈 문자열로 둔다.
