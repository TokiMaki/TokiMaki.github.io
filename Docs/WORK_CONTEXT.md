...
- 최근 서약 정책: `서약 결정 초월` 추천은 `oathTranscendRecommendations` 별도 필드와 `oathTranscend` sourceType으로 기존 `oathUpgrades`/서약 조율과 분리한다. target 서약 결정은 exact item search/detail 검증 후 생성하고, 딜러/버퍼별 유효 옵션만 표시하며, 에픽/태초 초월은 rarity별 최상위 1개와 태초 3칸 cap만 반영한다.
