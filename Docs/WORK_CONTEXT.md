...
- 최근 프론트 구조: `enchantEfficiencyScale.js`는 효율 색상 계산, `enchantSearchPanels.js`는 분석·후보 패널 표시, `enchantCreatureRows.js`는 크리쳐·아티팩트 추천 row 정규화, `enchantEquipmentLoadoutBoard.js`·`enchantOathLoadoutBoard.js`·`enchantAvatarLoadoutBoard.js`·`enchantBuffLoadoutBoard.js`는 읽기 전용 장비판 HTML 생성, `enchantAvatarEmblemViewHelpers.js`는 avatar·buff 공용 엠블렘 표현 helper, `enchantPortraitDetailPanel.js`는 detail panel binding, `enchantLoadoutNavigation.js`는 탭 markup과 delegated click 전환, `enchantDevelopmentTiming.js`는 개발 timing 상태·step·flush를 담당한다. 기존 표시·row·reset·탭 fallback과 timing ownership·stale·error/finally 계약은 유지한다.
- 다음 작업: 딜러·버퍼 시뮬레이터 meta/action 렌더링의 상태 의존성과 호출 경계를 확인해 다음 분리 단위를 정한다.
