...
- 최근 프론트 구조: `enchantEfficiencyScale.js`는 효율 색상 계산, `enchantSearchPanels.js`는 분석·후보 패널 표시, `enchantCreatureRows.js`는 크리쳐·아티팩트 추천 row 정규화, `enchantEquipmentLoadoutBoard.js`·`enchantOathLoadoutBoard.js`·`enchantAvatarLoadoutBoard.js`·`enchantBuffLoadoutBoard.js`는 읽기 전용 장비판 HTML 생성, `enchantAvatarEmblemViewHelpers.js`는 avatar·buff 공용 엠블렘 표현 helper를 담당한다. 기존 표시·row·binder 계약은 유지한다.
- 다음 작업: `enchantView.js`의 loadout tab·portrait detail binder가 공유하는 DOM attribute·event 계약과 안전한 분리 경계를 확인한다.
