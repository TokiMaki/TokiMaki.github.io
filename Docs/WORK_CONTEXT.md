...
- 최근 프론트 구조: `enchantEfficiencyScale.js`로 효율 색상 계산을, `enchantSearchPanels.js`로 분석·검색 후보 패널 표시를 분리했다. 기존 임계값, 문구, DOM 전환 순서와 `ctx.actions` 계약은 유지한다.
- 다음 작업: `enchantView.js`에서 크리쳐/아티팩트처럼 순수한 추천 row 정규화 한 영역을 다음 분리 대상으로 검토한다.
