...
- 최근 프론트 구조: `src/dnfHellTool/enchantEfficiencyScale.js`로 딜러/버퍼 효율 구간, 색상 보간, 추천 연결선 배경 계산을 분리했다. 임계값과 반환 문자열은 기존 구현을 그대로 유지한다.
- 다음 작업: 첫 분리의 동작 동일성을 확인한 뒤 `enchantView.js`에서 순수 후보 row 정규화 또는 읽기 전용 장비판 중 한 영역만 다음 대상으로 검토한다.
